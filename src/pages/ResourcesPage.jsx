...
import {
  ...
  HelpCircle, Award, ChevronDown, X,
  Calendar, Tag, Star, Zap, Database, CheckCircle,
  FolderOpen, BookMarked, PenTool, Cpu
} from "lucide-react";
...
function MatrixRain() {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (drops.length !== Math.floor(c.width / fs))
        drops = Array.from({ length: Math.floor(c.width / fs) }, () => Math.random() * -100);
  ...
  return <canvas ref={ref} style={{ position:"fixed",inset:0,zIndex:0,opacity:0.13,pointerEvents:"none" }} />;
}
  ...
  return (
    ...
    >
      ...
      <div style={{ display:"flex",alignItems:"flex-start",gap:10 }}>
        <div style={{ width:40,height:40,background:c.bg,border:`1px solid ${c.border}`,borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
...
export function ResourcesPage() {
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#020609",color:"#e8f5f0",position:"relative",overflowX:"hidden",paddingTop:60 }}>
      <MatrixRain />

      {/* HERO */}
      <section style={{ position:"relative",zIndex:1,padding:"60px 1.5rem 50px",textAlign:"center" }}>
        ...
        <div style={{ maxWidth:1300,margin:"0 auto" }}>
          <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"1.25rem" }}>
            ...
            {(search || activeType !== "all" || activeSubject !== "All Subjects") && (
              <button
                onClick={() => { setSearch(""); setActiveType("all"); setActiveSubject("All Subjects"); }}
        ...
        <div style={{ maxWidth:860,margin:"0 auto" }}>
          ...
          >
            ...
            <h2 style={{ fontFamily:"'Orbitron',monospace",fontSize:"clamp(1.2rem,3vw,2rem)",fontWeight:900,color:"#e8f5f0",marginBottom:"0.75rem" }}>
              HAVE RESOURCES TO{" "}
              <span style={{ background:"linear-gradient(135deg,#00e696,#00f5d4)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text" }}>SHARE?</span>
            </h2>
      ...
      <style>{`
        ...
        @media (max-width:640px) { section { padding-left:1rem !important; padding-right:1rem !important; } }
      `}</style>
    </div>
  );
}
